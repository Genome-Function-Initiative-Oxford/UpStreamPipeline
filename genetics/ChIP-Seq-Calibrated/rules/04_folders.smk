...
rule cleaning_folders:
    ...
    params:
        ...
        single_paired_folder=config["single_paired_folder"],
        ...
        bowtie2_rm=config["bowtie2_rm"],
        bowtie2=config["analysis_name"]+os.sep+config["bowtie2"],
        ...
        splitgenome_rm=config["splitgenome_rm"],
        splitgenome=config["analysis_name"]+os.sep+config["splitgenome"],

        readcounts_rm=config["readcounts_rm"],
        readcounts=config["analysis_name"]+os.sep+config["readcounts"],

        downsampling_factor_rm=config["downsampling_factor_rm"],
        downsampling_factor=config["analysis_name"]+os.sep+config["downsampling_factor"],
        
        downsampling_rm=config["downsampling_rm"],
        downsampling=config["analysis_name"]+os.sep+config["downsampling"],
    ...
    shell:
        """
            ...
            then
                ...
                if [ {params.bowtie2_rm} == "True"  ]
                then
                    rm -rf {params.bowtie2}
                fi
                ...
                if [ {params.splitgenome_rm} == "True"  ]
                then
                    rm -rf {params.splitgenome}
                fi
                if [ {params.readcounts_rm} == "True"  ]
                then
                    rm -rf {params.readcounts}
                fi
                if [ {params.downsampling_factor_rm} == "True"  ]
                then
                    rm -rf {params.downsampling_factor}
                fi
                if [ {params.downsampling_rm} == "True"  ]
                then
                    rm -rf {params.downsampling}
                fi
