...
modified: May 2023, Emily Georgiades to include -si superImpose
------------------------------------------------------------------------------------------'''

import os,sys
if sys.version_info[0] != 3 or sys.version_info[1] < 6:
    print("\nYou are using python" + str(sys.version_info[0]) + '.' + str(sys.version_info[1]) + " HiCPlotter needs python3.6.*!\n", file=sys.stderr)
    sys.exit()
...
def HiCplotter(files=[],names=[],resolution=100000,chromosome='',output='',histograms=[],histLabels=[],fillHist=[],histMax=[],verbose=False,fileHeader=0,fileFooter=0,matrixMax=0,histColors=[],barPlots=[],barLabels=[],plotGenes='',superImpose=False,anchor=0,anchorMax=0,\
    ...
    oExtension        (-ext)        : an extension name for the output file format - default png.
    ...
    for exp in range(0,rlength):
        ...
        if plotTriangular: 
            ...
            #ax2.set(adjustable='box-forced') # Removed as unsupported in matplotlib.
            if heatmapColor < 5:
        ...
        if not randomBins:
            ...
            ax1.xaxis.set_major_locator(matplotlib.ticker.FixedLocator(ticks))
    ...
    elif 'PNG' in plt.gcf().canvas.get_supported_filetypes_grouped().keys() or 'Joint Photographic Experts Group' in plt.gcf().canvas.get_supported_filetypes_grouped().keys():
        extension = '.png'
    else :
        extension = '.jpeg'
